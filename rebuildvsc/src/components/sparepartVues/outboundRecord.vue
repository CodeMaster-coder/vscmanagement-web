                    ...
                    <el-container width="80%">
                        <el-main>
                            ...
                            <el-table border size="mini" :row-style="{ height: '0' }" :data="pageData" width='100%'
                                ...
                                <el-table-column prop="apply_record__sparepart__name" label="备件名称" align="center"
                                    min-width='20%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__sparepart__type" label="备件型号" align="center"
                                    min-width='20%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__sparepart__brand" label="品牌" align="center"
                                    min-width='9%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__sparepart__unit" label="单位" align="center"
                                    min-width='9%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__sparepart__area__name" label="区域" align="center"
                                    min-width='9%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__sparepart__location" label="位置" align="center"
                                    min-width='9%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__sparepart__comment" label="备注" align="center"
                                    min-width='9%'>
                                </el-table-column>
...
export default {
    ...
    methods: {
        exportToExcel(){
            let that=this
            that.$exportToExcel(that.tableData,'备件出库记录.xlsx')
        },
        //修改每页的分页行数
        ...
        getAllSparepartStorage(workshop_id) {
            ...
            this.$api.sparepartAPI.getOutbound({ workshop: workshop_id })
                .then(function (res) {
                    if (res.data.code == 1) {
        ...
        deleteBtn(index) {
            ...
            that.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 8).then(
                res => {
                    if (res) {
                        this.$api.sparepartAPI.deleteOutbound({ singleOutbound: that.pageData[index] })
                            .then(function (res) {
                                if (res.data.code == 1) {
