                    ...
                    <el-container width="80%">
                        <el-main>
                            ...
                            <el-table border size="mini" :row-style="{ height: '0' }" :data="pageData" width='100%'>
                                <el-table-column prop="tool__name" label="工具名称" align="center" min-width='20%'>
                                </el-table-column>
                                <el-table-column prop="tool__type" label="工具型号" align="center" min-width='20%'>
                                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        getAllToolStorage(workshop_id) {
            ...
            this.$api.toolAPI.queryOutboundToolApply({ workshop: workshop_id })
                .then(function (res) {
                    if (res.data.code == 1) {
                        that.$message.success('获取工具出库申请成功！')
        ...
        approveBtn(index) {
            ...
            this.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 5).then(
                res => {
                    if (res) {
                        this.$api.toolAPI.approveOutbound({ approveOutbound: that.pageData[index], userNameId: JSON.parse(sessionStorage.getItem('userInfo')).key.id })
                            .then(function (res) {
                                if (res.data.code == 1) {
                                    that.getAllToolStorage(that.workshop)
                                    that.$message.success('获取工具出库批准成功！')
        ...
        deleteBtn(index) {
            ...
            this.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 5).then(
                res => {
                    if (res) {
            this.$api.toolAPI.deleteApply({ approveOutbound: that.pageData[index] })
                .then(function (res) {
                    if (res.data.code == 1) {
                        that.getAllToolStorage(that.workshop)
                        that.$message.success('删除工具出库申请成功！')
                    } else {
                        that.$message.error(res.data.msg)
