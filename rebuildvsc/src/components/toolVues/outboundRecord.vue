                    ...
                    <el-container width="80%">
                        <el-main>
                            ...
                            <el-table border size="mini" :row-style="{ height: '0' }" :data="pageData" width='100%'
                                ...
                                <el-table-column prop="approve_date" label="审批时间" align="center"
                                    min-width='13%'>
                                </el-table-column>
...
export default {
    ...
    methods: {
        exportToExcel(){
            let that=this
            that.$exportToExcel(that.tableData,'工具出库记录.xlsx')
        },
        //修改每页的分页行数
        ...
        getAllToolStorage(workshop_id) {
            ...
            this.$api.toolAPI.getOutbound({ workshop: workshop_id })
                .then(function (res) {
                    if (res.data.code == 1) {
                        that.$message.success('获取工具入库记录成功！')
        ...
        deleteBtn(index) {
            ...
            this.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 5).then(
                res => {
                    if (res) {
            this.$api.toolAPI.deleteOutbound({ singleOutbound: that.pageData[index] })
                .then(function (res) {
                    if (res.data.code == 1) {
                        that.$message.success('删除工具入库记录成功！')
