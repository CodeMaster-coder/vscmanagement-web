...
export default {
    ...
    methods: {
        exportToExcel(){
            let that=this
            that.$exportToExcel(that.tableData,'工具入库记录.xlsx')
        },
        //修改每页的分页行数
        ...
        getAllToolStorage(workshop_id) {
            ...
            this.$api.toolAPI.getInbound({ workshop: workshop_id })
                .then(function (res) {
                    if (res.data.code == 1) {
        ...
        deleteBtn(index) {
            ...
            this.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 5).then(
                res => {
                    if (res) {
            this.$api.toolAPI.deteleInbound({ singleInbound: that.pageData[index] })
                .then(function (res) {
                    if (res.data.code == 1) {
