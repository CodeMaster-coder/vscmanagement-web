                    ...
                    <el-container width="80%">
                        <el-main>
                            ...
                            <el-table border size="mini" :row-style="{ height: '0' }" :data="pageData" width='100%'
                                ...
                                <el-table-column prop="apply_record__material__name" label="耗材名称/name" align="center" 
                                min-width='20%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__material__type" label="耗材型号/type" align="center" 
                                min-width='20%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__material__unit" label="单位/unit" align="center" 
                                min-width='8%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__apply_quantity" label="出库数量/out-quantity" align="center" 
                                min-width='9%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__apply_userName__name" label="申请人/apply user" align="center"
                                    min-width='10%'>
                                </el-table-column>
                                <el-table-column prop="apply_record__apply_date" label="申请时间/apply date" align="center"
                                    min-width='13%'>
                                </el-table-column>
                                <el-table-column prop="approve_username__name" label="审批人/approve user" align="center"
                                    min-width='10%'>
                                </el-table-column>
                                <el-table-column prop="approve_date" label="审批时间/approve date" align="center"
                                    min-width='13%'>
                                </el-table-column>
...
export default {
    ...
    methods: {
        exportToExcel(){
            let that=this
            that.$exportToExcel(that.tableData,'耗材出库记录.xlsx')
        },
        //修改每页的分页行数
        ...
        getAllMaterialStorage(workshop_id) {
            ...
            this.$api.materialAPI.getOutbound({ workshop: workshop_id })
                .then(function (res) {
                    if (res.data.code == 1) {
        ...
        deleteBtn(index) {
            ...
            this.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 4 ).then(
                res=>{
                    if (res) {
            this.$api.materialAPI.deleteOutbound({ singleOutbound: that.pageData[index] })
                .then(function (res) {
                    if (res.data.code == 1) {
