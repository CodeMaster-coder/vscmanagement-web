                    ...
                    <el-container width="80%">
                        <el-main>
                            ...
                            <el-table border size="mini" :row-style="{ height: '0' }" :data="pageData" width='100%'>
                                <el-table-column prop="material__name" :label="$t('messages.storage.materialName')" align="center" min-width='20%'>
                                </el-table-column>
                                <el-table-column prop="material__type" :label="$t('messages.storage.materialType')" align="center" min-width='20%'>
                                </el-table-column>
                                <el-table-column prop="material__unit" :label="$t('messages.storage.materialUnit')" align="center" min-width='9%'>
                                </el-table-column>
                                <el-table-column prop="apply_quantity" :label="$t('messages.storage.applyQuatity')" align="center" min-width='9%'>
                                </el-table-column>
                                <el-table-column prop="apply_date" :label="$t('messages.outbound.applyDate')" align="center" min-width='15%'>
                                </el-table-column>
                                <el-table-column prop="apply_userName__name" :label="$t('messages.outbound.applyUser')" align="center" min-width='9%'>
                                </el-table-column>
                                <el-table-column :label="$t('messages.storage.edit')" size="mini" align="center" min-width='15%'>
                                    <template slot-scope="scope">
                                        <el-button type="primary" icon="el-icon-finished" size="mini"
                                            @click="approveBtn(scope.$index)" v-preventReClick="1000">
                                            {{ $t('messages.outbound.approve') }}
                                        </el-button>
                                        <el-button type="primary" icon="el-icon-delete" size="mini"
                                            @click="deleteBtn(scope.$index)" v-preventReClick="1000">
                                            {{ $t('messages.outbound.reject') }}
                                        </el-button>
                                    </template>
...
export default {
    ...
    methods: {
        ...
        getAllMaterialStorage(workshop_id) {
            ...
            this.$api.materialAPI.queryOutboundMaterialApply({ workshop: workshop_id })
                .then(function (res) {
                    if (res.data.code == 1) {
        ...
        approveBtn(index) {
            ...
            this.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 4 ).then(
                res=>{
                    if (res) {
            this.$api.materialAPI.approveOutbound({ approveOutbound: that.pageData[index], userNameId: JSON.parse(sessionStorage.getItem('userInfo')).key.id })
                .then(function (res) {
                    if (res.data.code == 1) {
                        that.getAllMaterialStorage(that.workshop)
                        that.$message.success(that.$t('messages.outbound.approveApplySuccess'))
        ...
        deleteBtn(index) {
            ...
            this.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 4 ).then(
                res=>{
                    if (res) {
            this.$api.materialAPI.deleteApply({ approveOutbound: that.pageData[index] })
                .then(function (res) {
                    if (res.data.code == 1) {
                        that.getAllMaterialStorage(that.workshop)
                        that.$message.success(that.$t('messages.outbound.rejectApplySuccess'))
                    } else {
                        that.$message.error(res.data.msg)
                    }
                })
