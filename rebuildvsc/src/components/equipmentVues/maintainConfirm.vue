                            ...
                            <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" width="50%"
                                ...
                                <el-form :model="singleTool" :rules="rules" ref="dialogForm" :inline="true">
                                    ...
                                    <el-form-item label="维修措施：" :label-width="formLabelWidth" prop="solution">
                                        <el-input :disabled="true" v-model="singleTool.solution" autocomplete="off"
                                    ...
                                    <el-form-item label="更换备件：" :label-width="formLabelWidth">
                                        <template>
                                            <el-radio :disabled="true" v-model="singleTool.part_change"
                                    ...
                                    <el-form-item label="截止日期：" :label-width="formLabelWidth" prop="deadline">
                                        <div class="block">
                                            <el-date-picker value-format="yyyy-MM-dd" format="yyyy-MM-dd"
                                                :disabled="true" v-model="singleTool.deadline" type="date"
                                                placeholder="选择截止日期">
                                            </el-date-picker>
                                        </div>
                                    </el-form-item>
                                    ...
                                    <el-form-item label="是否修复：" :label-width="formLabelWidth">
                                        <template>
                                            <el-radio :disabled="true" v-model="singleTool.repaired_or_not"
                                                :label="true">是</el-radio>
                                ...
                                <div slot="footer" class="dialog-footer">
                                    <el-button  type="primary" size="mini" @click="approveBtn()">
                                        批准
                                    </el-button>
                                    <el-button  type="primary" size="mini" @click="rejectBtn()">
                                        驳回
                                    </el-button>
                                </div>
                            </el-dialog>
...
export default {
    ...
    methods: {
        ...
        getAllToolStorage(workshop) {
            ...
            this.$api.equipmentAPI.getAllFinishMaintain({ workshop: workshop })
                .then(function (res) {
                    if (res.data.code == 1) {
                        that.$message.success('获取已结束的维修成功！')
        ...
        approveBtn() {
            ...
            let that = this
                    that.$api.equipmentAPI.approveMaintainFinish({ singleTool: that.singleTool, userId: JSON.parse(sessionStorage.getItem('userInfo')).key.id })
                        .then(function (res) {
                            console.log(res)
                            if (res.data.code == 1) {
                                that.$message.success('维修结束审批成功！')
                                that.cancelDialog()
        ...
        rejectBtn() {
            ...
            that.$api.equipmentAPI.rejectMaintainFinish({ singleTool: that.singleTool })
                .then(function (res) {
                    // console.log(res)
                    if (res.data.code == 1) {
                        that.$message.success('驳回申请成功！')
                        that.cancelDialog()
                        that.getAllToolStorage(that.workshop)
                    } else {
                        that.$message.error(res.data.msg)
