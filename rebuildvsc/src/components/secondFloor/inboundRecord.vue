                    ...
                    <el-container width="80%">
                        <el-main>
                            ...
                            <el-table border size="mini" :row-style="{ height: '0' }" :data="pageData" width='100%'
                                ...
                                <el-table-column prop="apply_record__apply_quantity" label="入库数量" align="center"
                                    min-width='9%'>
                                </el-table-column>
...
export default {
    ...
    methods: {
        exportToExcel(){
            let that=this
            that.$exportToExcel(that.tableData,'二层平台入库记录.xlsx')
        },
        //修改每页的分页行数
        ...
        getAllSparepartStorage(workshop_id) {
            ...
            this.$api.secondFloorAPI.getAllInboundApprove({ workshop: workshop_id })
                .then(function (res) {
                    if (res.data.code == 1) {
                        that.$message.success('获取设备入库记录成功！')
        ...
        deleteBtn(index) {
            ...
            that.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 2).then(
                res => {
                    if (res) {
                        this.$api.secondFloorAPI.deleteInboundRecord({ singleInbound: that.pageData[index] })
                            .then(function (res) {
                                if (res.data.code == 1) {
                                    that.$message.success('删除设备入库记录成功！')
