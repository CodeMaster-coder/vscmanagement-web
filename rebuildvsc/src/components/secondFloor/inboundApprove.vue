...
export default {
    ...
    methods: {
        ...
        getAllToolStorage(workshop_id) {
            ...
            this.$api.secondFloorAPI.getInboundApply({ workshop: workshop_id })
                .then(function (res) {
                    if (res.data.code == 1) {
                        that.$message.success('获取二层设备入库申请成功！')
        ...
        approveBtn(index) {
            ...
            that.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 2).then(
                res => {
                    if (res) {
                        this.$api.secondFloorAPI.inboundApprove({ approveInbound: that.pageData[index], userNameId: JSON.parse(sessionStorage.getItem('userInfo')).key.id })
                            .then(function (res) {
                                if (res.data.code == 1) {
                                    that.getAllToolStorage(that.workshop)
                                    that.$message.success('二层平台入库申请审批成功！')
            ...
            that.$checkUserAuth(JSON.parse(sessionStorage.getItem('userInfo')).key.id, 2).then(
                res => {
                    if (res) {
                        ...
                        this.$api.secondFloorAPI.deleteInboundApply({ approveInbound: that.pageData[index] })
                            .then(function (res) {
                                if (res.data.code == 1) {
                                    that.getAllToolStorage(that.workshop)
